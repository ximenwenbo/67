<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:70:"F:\www\php68\public/../application/admin\view\goods\index_seckill.html";i:1535013329;s:49:"F:\www\php68\application\admin\view\pub\head.html";i:1532091344;s:63:"F:\www\php68\application\admin\view\goods\js_index_seckill.html";i:1535013239;s:49:"F:\www\php68\application\admin\view\pub\foot.html";i:1532090900;}*/ ?>
<!--_meta 作为公共模版分离出去-->
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
    <meta name="renderer" content="webkit|ie-comp|ie-stand">
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta name="viewport" content="width=device-width,initial-scale=1,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no" />
    <meta http-equiv="Cache-Control" content="no-siteapp" />
    <link rel="Bookmark" href="/favicon.ico" >
    <link rel="Shortcut Icon" href="/favicon.ico" />
    <!--[if lt IE 9]>
    <script type="text/javascript" src="<?php echo config('admin_lib'); ?>html5shiv.js"></script>
    <script type="text/javascript" src="<?php echo config('admin_lib'); ?>respond.min.js"></script>
    <![endif]-->
    <link rel="stylesheet" type="text/css" href="<?php echo config('admin_static'); ?>h-ui/css/H-ui.min.css" />
    <link rel="stylesheet" type="text/css" href="<?php echo config('admin_static'); ?>h-ui.admin/css/H-ui.admin.css" />
    <link rel="stylesheet" type="text/css" href="<?php echo config('admin_lib'); ?>Hui-iconfont/1.0.8/iconfont.css" />
    <link rel="stylesheet" type="text/css" href="<?php echo config('admin_static'); ?>h-ui.admin/skin/default/skin.css" id="skin" />
    <link rel="stylesheet" type="text/css" href="<?php echo config('admin_static'); ?>h-ui.admin/css/style.css" />
    <!--[if IE 6]>
    <script type="text/javascript" src="<?php echo config('admin_lib'); ?>DD_belatedPNG_0.0.8a-min.js" ></script>
    <script>DD_belatedPNG.fix('*');</script>
    <![endif]-->
    <!--/meta 作为公共模版分离出去-->
    <!--_footer 作为公共模版分离出去-->
    <script type="text/javascript" src="<?php echo config('admin_lib'); ?>jquery/1.9.1/jquery.min.js"></script>
    <script type="text/javascript" src="<?php echo config('admin_lib'); ?>layer/2.4/layer.js"></script>
    <script type="text/javascript" src="<?php echo config('admin_static'); ?>h-ui/js/H-ui.min.js"></script>
    <script type="text/javascript" src="<?php echo config('admin_static'); ?>h-ui.admin/js/H-ui.admin.js"></script> <!--/_footer 作为公共模版分离出去-->

<title>秒杀商品管理</title>

</head>
<body>

<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 秒杀商品中心 <span class="c-gray en">&gt;</span> 秒杀商品管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">

	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>

		<a href="javascript:;" onclick="member_add('添加秒杀商品','<?php echo url('tianjia'); ?>','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加秒杀商品</a>

	</span> <span class="r">共有数据：<strong>88</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				<th width="25"><input type="checkbox" name="" value=""></th>
				<th width="30">ID</th>
				<th width="100">名称</th>
				<th width="60">秒杀价格</th>
				<th width="50">秒杀数量</th>
				<th width="40">开始时间</th>
				<th width="70">结束时间</th>
				<th width="150">倒计时</th>
			</tr>
		</thead>
		<tbody>
			<?php if(is_array($seckillinfos) || $seckillinfos instanceof \think\Collection || $seckillinfos instanceof \think\Paginator): $i = 0; $__LIST__ = $seckillinfos;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
			<tr class="text-c">
				<td><input type="checkbox" value="1" name=""></td>
				<td><?php echo $v['goods_id']; ?></td>
				<td><?php echo $v['goods_name']; ?></td>
				<td><?php echo $v['goods_price_seckill']; ?></td>
				<td><?php echo $v['goods_number_seckill']; ?></td>
				<td><?php echo date('Y-m-d H:i:s',$v['start_time']); ?></td>
				<td><?php echo date('Y-m-d H:i:s',$v['end_time']); ?>
					<input type="hidden" id="goodssec_<?php echo $v['goods_id']; ?>" value="<?php echo $v['end_time']; ?>" />
				</td>
				<td id="backtime_<?php echo $v['goods_id']; ?>">倒计时</td>
			</tr>
			<?php endforeach; endif; else: echo "" ;endif; ?>
		</tbody>
	</table>
	</div>
</div>
<script type="text/javascript">
//页面加载完毕，给各个秒杀商品计算“倒计时”信息显示

$(function(){
    $("[id^='goodssec_']").each(function(i,v){
        //v: 被遍历出来的具体dom元素对象
        var end_time = $(v).val(); //获得商品秒杀结束时间
        //设置当前时间到js变量中
        var now_time = <?php echo time(); ?>;

        //每秒钟显示一次"倒计时"时间
        window.setInterval(function(){
            var cha = end_time-now_time;
            var show_back = getDHMS(cha);  //计算好的倒计时信息
            now_time++;

            //显示倒计时到页面
            var goods_id = ($(v).attr('id').split('_'))[1]//计算当前goods_id
            $('#backtime_'+goods_id).html(show_back);
        },1000);
    });
});

/**
 * @param difftime 时间差值
 * @return 返回 “天 小时 分钟 秒” 的信息
 */
function getDHMS(difftime){
    var day     = 60*60*24;     //1天的时间戳
    var hour    = 60*60;        //1小时的时间戳
    var minute  = 60;           //1分钟的时间戳

    var days        = parseInt(difftime / day);              //计算有几个天
    var hours       = parseInt(difftime % day / hour);       //抛开天，剩余多少小时
    var minutes     = parseInt(difftime % hour / minute);    //抛开小时，剩余几分钟
    var seconds     = parseInt(difftime % minute);           //抛开分钟，剩余几秒
    return days+'天'+hours+'小时'+minutes+'分钟'+seconds+'秒';
}

</script>
</body>
</html>


























